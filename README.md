<div align="center">

<!-- Animated Header Banner -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,50:302b63,100:24243e&height=200&section=header&text=Shanuka%20Siriwardana&fontSize=50&fontColor=ffffff&fontAlignY=38&desc=Data%20Analyst%20%7C%20AI%20%26%20Automation%20Expert&descColor=a78bfa&descSize=18&animation=fadeIn" width="100%"/>

<!-- Typing Animation -->
<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&duration=3000&pause=800&color=A78BFA&center=true&vCenter=true&multiline=false&width=700&lines=Transforming+data+into+actionable+insights+%F0%9F%93%8A;Building+intelligent+automation+pipelines+%F0%9F%A4%96;Architecting+AI-powered+solutions+%E2%9A%A1;Making+complex+data+simple+%E2%9C%A8" alt="Typing SVG" />
</a>

<br/>

<!-- Profile views + followers badges -->
<img src="https://komarev.com/ghpvc/?username=Shannav&style=for-the-badge&color=7c3aed&label=PROFILE+VIEWS" />
&nbsp;
<img src="https://img.shields.io/github/followers/ShannaSiriwardana?style=for-the-badge&color=7c3aed&labelColor=1a1a2e&label=FOLLOWERS" />

</div>

---

## 🧠 About Me

```python
class DataSpecialist:
    def __init__(self):
        self.name        = "Shanuka Siriwardana"
        self.role        = ["Data Analyst", "AI & Automation Expert", "Data Specialist"]
        self.location    = "Sri Lanka 🇱🇰"
        self.focus       = ["Machine Learning", "Process Automation", "Data Infrastructure"]
        self.superpower  = "Turning messy data into strategic decisions"

    def current_work(self):
        return {
            "learning"  : "Advanced MLOps & LLM fine-tuning",
            "building"  : "AI-powered automation pipelines",
            "exploring" : "Generative AI & Agentic workflows"
        }

me = DataSpecialist()
print(me.current_work())
```

---

## 🚀 Tech Stack

<div align="center">

### 📊 Data & Analytics
![Python](https://img.shields.io/badge/Python-0D1117?style=for-the-badge&logo=python&logoColor=3776AB)
![SQL](https://img.shields.io/badge/SQL-0D1117?style=for-the-badge&logo=postgresql&logoColor=336791)
![Pandas](https://img.shields.io/badge/Pandas-0D1117?style=for-the-badge&logo=pandas&logoColor=150458)
![NumPy](https://img.shields.io/badge/NumPy-0D1117?style=for-the-badge&logo=numpy&logoColor=013243)
![Jupyter](https://img.shields.io/badge/Jupyter-0D1117?style=for-the-badge&logo=jupyter&logoColor=F37626)

### 🤖 AI & Machine Learning
![TensorFlow](https://img.shields.io/badge/TensorFlow-0D1117?style=for-the-badge&logo=tensorflow&logoColor=FF6F00)
![Scikit-learn](https://img.shields.io/badge/Scikit--Learn-0D1117?style=for-the-badge&logo=scikitlearn&logoColor=F7931E)
![PyTorch](https://img.shields.io/badge/PyTorch-0D1117?style=for-the-badge&logo=pytorch&logoColor=EE4C2C)
![OpenAI](https://img.shields.io/badge/OpenAI-0D1117?style=for-the-badge&logo=openai&logoColor=412991)
![LangChain](https://img.shields.io/badge/LangChain-0D1117?style=for-the-badge&logo=chainlink&logoColor=1C3C3C)

### 📈 Visualization & BI
![Power BI](https://img.shields.io/badge/Power%20BI-0D1117?style=for-the-badge&logo=powerbi&logoColor=F2C811)
![Tableau](https://img.shields.io/badge/Tableau-0D1117?style=for-the-badge&logo=tableau&logoColor=E97627)
![Plotly](https://img.shields.io/badge/Plotly-0D1117?style=for-the-badge&logo=plotly&logoColor=3F4F75)
![Grafana](https://img.shields.io/badge/Grafana-0D1117?style=for-the-badge&logo=grafana&logoColor=F46800)

### ☁️ Cloud & Infrastructure
![AWS](https://img.shields.io/badge/AWS-0D1117?style=for-the-badge&logo=amazonaws&logoColor=FF9900)
![GCP](https://img.shields.io/badge/GCP-0D1117?style=for-the-badge&logo=googlecloud&logoColor=4285F4)
![Docker](https://img.shields.io/badge/Docker-0D1117?style=for-the-badge&logo=docker&logoColor=2496ED)
![Airflow](https://img.shields.io/badge/Airflow-0D1117?style=for-the-badge&logo=apacheairflow&logoColor=017CEE)

### 🌐 Web & Automation
![React](https://img.shields.io/badge/React-0D1117?style=for-the-badge&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-0D1117?style=for-the-badge&logo=nodedotjs&logoColor=339933)
![n8n](https://img.shields.io/badge/n8n-0D1117?style=for-the-badge&logo=n8n&logoColor=EA4B71)
![Make](https://img.shields.io/badge/Make-0D1117?style=for-the-badge&logo=make&logoColor=6D00CC)

</div>

---

## 📊 GitHub Analytics

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=Shannav&show_icons=true&theme=transparent&hide_border=true&title_color=a78bfa&icon_color=7c3aed&text_color=e2e8f0&bg_color=0d1117&include_all_commits=true&count_private=true"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Shannav&layout=compact&theme=transparent&hide_border=true&title_color=a78bfa&text_color=e2e8f0&bg_color=0d1117&langs_count=8"/>

</div>

<div align="center">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=Shannav&theme=transparent&hide_border=true&ring=7c3aed&fire=a78bfa&currStreakLabel=a78bfa&sideLabels=e2e8f0&sideNums=e2e8f0&dates=94a3b8&background=0d1117" alt="GitHub Streak" />

</div>

<!-- Activity Graph -->
<div align="center">
<img src="https://github-readme-activity-graph.vercel.app/graph?username=Shannav&bg_color=0d1117&color=a78bfa&line=7c3aed&point=ffffff&area=true&hide_border=true&area_color=7c3aed" width="95%"/>
</div>

---

## 🏆 GitHub Trophies

<div align="center">
<img src="https://github-profile-trophy.vercel.app/?username=Shannav&theme=discord&no-frame=true&no-bg=true&margin-w=6&column=7" alt="GitHub Trophies" />
</div>

---

## 🛠️ Featured Projects

<div align="center">

<a href="https://github.com/ShannaSiriwardana/POS_System_AI">
  <img align="center" src="https://github-readme-stats.vercel.app/api/pin/?username=Shannav&repo=POS_System_AI&theme=transparent&hide_border=true&title_color=a78bfa&text_color=e2e8f0&bg_color=0d1117&icon_color=7c3aed" />
</a>
&nbsp;
<a href="https://github.com/ShannaSiriwardana/testing-">
  <img align="center" src="https://github-readme-stats.vercel.app/api/pin/?username=Shannav&repo=testing-&theme=transparent&hide_border=true&title_color=a78bfa&text_color=e2e8f0&bg_color=0d1117&icon_color=7c3aed" />
</a>

</div>

<br/>

| 🚀 Project | 📋 Description | 🛠 Tech Stack | 🔗 Link |
|:---|:---|:---|:---:|
| **POS System AI** | AI-powered Point of Sale system with intelligent analytics and automation | Python, ML, React | [→ Repo](https://github.com/ShannaSiriwardana/POS_System_AI) |
| **Shannav** | Automation & data pipeline project | Python, Node.js | [→ Repo](https://github.com/ShannaSiriwardana/Shannav) |
| **Data Dashboard** | Interactive analytics dashboard with real-time insights | React, D3.js, AWS | `Coming Soon` |
| **ML Pipeline** | End-to-end machine learning pipeline with MLOps | Python, Airflow, Docker | `Coming Soon` |

---

## 📈 Contribution Metrics

<div align="center">

```
📅 2024 — Contribution Heatmap
───────────────────────────────────────────────────────
Jan  Feb  Mar  Apr  May  Jun  Jul  Aug  Sep  Oct  Nov  Dec
▓▓▓  ░▓▓  ▓▓▓  ▓░▓  ▓▓░  ░▓▓  ▓▓▓  ▓▓░  ░▓▓  ▓▓▓  ▓░▓  ▓▓▓
▓░▓  ▓▓░  ░▓▓  ▓▓▓  ░░▓  ▓▓▓  ░▓░  ▓▓▓  ▓▓░  ░▓▓  ▓▓▓  ▓░░
───────────────────────────────────────────────────────
░ = low activity   ▓ = high activity
```

</div>

---

## 💡 What I Do

<table>
<tr>
<td width="50%">

### 📊 Data Analysis & Engineering
- Complex dataset analysis & cleaning
- ETL pipeline design & optimization
- Data warehouse architecture
- Statistical modeling & forecasting

</td>
<td width="50%">

### 🤖 AI & Automation
- Machine learning model development
- LLM integration & prompt engineering
- RPA & workflow automation
- AI-powered application development

</td>
</tr>
<tr>
<td width="50%">

### 📈 Business Intelligence
- Interactive dashboard creation
- KPI tracking & reporting
- Data storytelling & visualization
- Executive-level insights delivery

</td>
<td width="50%">

### ☁️ Infrastructure & MLOps
- Cloud data platform management
- Model deployment & monitoring
- CI/CD for data pipelines
- Scalable architecture design

</td>
</tr>
</table>

---

## 🌐 Connect With Me

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=0A66C2)](https://linkedin.com/in/shanuka-siriwardana)
[![Email](https://img.shields.io/badge/Email-0D1117?style=for-the-badge&logo=gmail&logoColor=EA4335)](mailto:your.email@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-0D1117?style=for-the-badge&logo=vercel&logoColor=ffffff)](https://your-portfolio.dev)
[![Kaggle](https://img.shields.io/badge/Kaggle-0D1117?style=for-the-badge&logo=kaggle&logoColor=20BEFF)](https://kaggle.com/shanukasiriwardana)

</div>

---

<div align="center">

<!-- Footer wave -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:24243e,50:302b63,100:0f0c29&height=100&section=footer" width="100%"/>

<sub>⚡ Powered by data, driven by curiosity — <strong>Shanuka Siriwardana</strong></sub>

</div>
